...
public class PhysicsThread extends Thread {
	...
	/*
	 * public synchronized boolean catchCollisionRectangle(Rectangle R1, Rectangle
	 * R2) { if(R1.intersects(R2)) { return true; } return false; }
	 */
	...
	public synchronized boolean catchAdvancedCollisionPacManPacGum(Ellipse2D.Float var_pacman,
			Ellipse2D.Float var_food) {
		if (((var_pacman.getWidth() / 2) + (var_food.getWidth() / 2)) >= ((var_pacman.x + (var_pacman.getWidth() / 2)
				- var_food.x + (var_food.getWidth() / 2)))
				|| ((var_pacman.getHeight() / 2) + (var_food.getHeight() / 2)) >= ((var_pacman.y
						+ (var_pacman.getHeight() / 2) - var_food.y + (var_food.getHeight() / 2)))) {
			return true;
		}
		return false;
	}
	...
	public synchronized boolean catchCollisionPacManGhost(PacMan var_pacMan, List<Ghost> ghost) {
		...
		while (iter.hasNext()) {
			...
			if (pacmanRectangle.intersects(x.getRectangle())) {
				...
				if (advancedCatchCollisionTactics(pacMan.getEllipse(), x.getAdvancedLowerRectangle(),
						x.getAdvancedTopArc())) {
					x.setEaten(true);
					return true;
				}
	...
	private synchronized boolean advancedCatchCollisionTactics(Ellipse2D.Float var_pacman, Rectangle var_ghostlower,
			Arc2D.Float var_ghostUpper) {
		if (var_pacman.intersects(var_ghostlower)) {
			// System.out.println("Advanced Rectangle Collision");
			return true;
		} else if (((var_pacman.getWidth() / 2) + (var_ghostUpper.getWidth() / 2)) >= ((var_pacman.x
				+ (var_pacman.getWidth() / 2) - var_ghostUpper.getCenterX()))
				&& ((var_pacman.getHeight() / 2) + (var_ghostUpper.getHeight() / 2)) >= ((var_pacman.y
						+ (var_pacman.getHeight() / 2) - var_ghostUpper.getCenterY()))) {
		...
		while (isRunning) {
			try {
				...
				if (catchCollisionPacManGhost(pacMan, ghost)) {
					// devrait �tre dans le controlleur, la thread ne devrait pas contenir de
					// logique de code � l'int�rieur, mais seulement voir les collisions.
